...
struct S_VertexIn {
	...
	float3 shape_pos : S_POS;
	float3 shape_norm : S_NORM;
};
...
S_VertexOut main(S_VertexIn IN)
{
	...
	float t = shape_time[0];
	float4 pos = float4(lerp(IN.pos, IN.shape_pos, float3(t,t,t)), 1);
	float4 out_pos = mul(pos, m_world_matrix);
